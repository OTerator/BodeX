...
static void testConfigRoundTrip()
{
    ...
    CHECK(r3.prefs.autosaveSec == 30.0);
    ...
    // Out-of-range prefs are clamped to sane ranges on read; unknown keys ignored.
    AppConfig clamp;
    CHECK(configFromJsonString(
        "{ \"prefs\": { \"theme\": 9, \"stepSize\": 0.0, \"uiScale\": 99.0 } }", clamp));
